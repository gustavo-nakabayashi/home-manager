...
if [[ $# -eq 1 ]]; then
    ...
    GW_SEARCH_PATHS="${XDG_CONFIG_HOME:-$HOME/.config}/gw/search-paths"
    if [[ -f "$GW_SEARCH_PATHS" ]]; then
        search_dirs=()
        while IFS= read -r dir; do
            [[ -z "$dir" || "$dir" == \#* ]] && continue
            dir="${dir/#\~/$HOME}"
            [[ -d "$dir" ]] && search_dirs+=("$dir")
        done < "$GW_SEARCH_PATHS"
    else
        search_dirs=(~/ ~/Programs/video-peel ~/.config ~/Programs ~/Programs/bridge ~/Programs/bridge/bridge-app-ui ~/homelab ~/Exercism/ ~/csprimer)
    fi
...
# Check for pinned worktree - read from gw config
pins_file="${XDG_CONFIG_HOME:-$HOME/.config}/gw/pins"
